...
@Description : create a efficientnet_advprop model. you can update code,use B0-B8
              支持对抗训练
              advprop 这个参数控制
...
class EfficientnetAutoAdvprop(nn.Module):

    def __init__(self, cfg):
        super(EfficientnetAutoAdvprop, self).__init__()
        self.cfg = cfg
        self.createModel()
    ...
    def createModel(self):
        ...
        if self.cfg['pretrained']:
            self.model= EfficientNet.from_pretrained('efficientnet-b7', weights_path=self.cfg['pretrained'],
                                                  num_classes=self.cfg['class_number'], advprop=True)
        return self.model
